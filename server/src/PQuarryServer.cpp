...
void PQuarryServer::start()
{
    ...
    // If the client disconnects from the SSL socket, the server will terminate
    // because of a broken pipe, so we have to set our own handler
...
void PQuarryServer::start_sniffer()
{
    ...
    std::thread th([this](){
        ...
        catch(const std::exception& e)
        {
            ...
            kill(getpid(), SIGPIPE); // Calls the sigpipe handler in the main thread
        }
    });
